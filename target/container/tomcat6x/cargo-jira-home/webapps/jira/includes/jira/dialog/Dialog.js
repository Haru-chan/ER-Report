...
JIRA.Dialog = AJS.Control.extend({
    ...
    show: function () {
        ...
        //Content is cached when stacked, so lets treat it as such.
        if (this.options.type !== "blank" && !this.$content && this.stacked !== true) {
            this._setContent();
        } else {
            $popup.show();
            this._positionInCenter();
        ...
        AJS.$(this).trigger("Dialog.show", [this.$popup]);
        AJS.$(AJS).trigger("Dialog.show", [this.$popup]);
    ...
    hide: function (undim, options) {
        ...
        JIRA.trigger(beforeHideEvent, [this.$popup, options.reason]);
        AJS.$(this).trigger(beforeHideEvent, [this.$popup, options.reason]);
        ...
        // fire hide events
        AJS.$(document).trigger("hideAllLayers", [this.$popup, options.reason]);
        AJS.$(this).trigger("Dialog.hide", [this.$popup, options.reason]);
        JIRA.trigger("Dialog.hide", [this.$popup, options.reason]);
