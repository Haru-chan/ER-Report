...
JIRA.Mention = AJS.Control.extend({
    ...
    generateSuggestions: function (data) {
        ...
        $suggestions =  AJS.$(JIRA.Templates.mentionsSuggestions({
            suggestions: data,
            hasSuggestions: data && data.length
        }));
    ...
    _keyUp: function() {
        if (this.listening) {
            ...
            } else if (this.layerController.isVisible()) {
                this._hide();
            }
        }
    },
                    ...
                    if (this.layerController.isVisible()) {
                        ...
                        this.$textarea.one("keyup", function(keyUpEvent) {
                            if (keyUpEvent.keyCode === jQuery.ui.keyCode.ESCAPE) {
                                ...
                                jQuery(AJS).trigger("Mention.afterHide");
                            }
                        });
                    }
...
JIRA.Mention.Matcher = {
    ...
    getUserNameFromCurrentWord: function(text, caretPosition) {
        ...
        var currentWord = before.substr(lastWordStartIndex);
