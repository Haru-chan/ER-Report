...
JIRA.ViewIssue = (function () {
    ...
    var issueTabs = {
        domReady: function() {
            JIRA.ViewIssueTabs.onTabReady(setupMouseoverBehaviour);
            JIRA.ViewIssueTabs.onTabReady(JIRA.userhover);
            JIRA.ViewIssueTabs.domReady();
        }
    };
...
jQuery(function (){
    ...
    jQuery(document).bind('keydown', function (e) {
        ...
        if (e.keyCode === jQuery.ui.keyCode.ESCAPE && AJS.InlineLayer.current) {
            preventEscClosingComments = true;
            jQuery(document).one('Mention.afterHide', function (e) {
                preventEscClosingComments = false;
            });
        }
        if (e.keyCode === jQuery.ui.keyCode.ESCAPE && !AJS.InlineLayer.current && !preventEscClosingComments) {
    ...
    var addCommentContainer = jQuery("#addcomment .mod-content");
    ...
    AJS.$("#footer-comment-button").click(function (e) {
        ...
        if (elem.hasClass("active")) {
            ...
            elem.addClass("active");
            jQuery("form#issue-comment-add").appendTo("#addcomment .mod-content");
            // Workaround for IE8 not focusing input
            setTimeout(function() {
    ...
    AJS.$("#issue-comment-add").submit(function () {
        var $this = AJS.$(this);
